...
% 中文字体配置（revtex4-2 自带 section 格式，无需额外 ctexset 配置）
...
CSQIT 与因果集理论的结构性差异如\cref{tab:causal_set} 所示。
...
class AxiomA' (M C : Type*) where
  ...
  compose_input : ∀ α β, input (compose α β) = input α ++ input β
  compose_output' : ∀ α β, output (compose α β)
                      = combine (output α) (output β)
  compose_assoc : ∀ α β γ,
    compose (compose α β) γ = compose α (compose β γ)
\end{verbatim}}
...
两面性二一定理是\textbf{离散互补性原理}：因果结构（output）与量子信息（amplitude）在标准理论中不可同时完全实现。两面性定理与连续量子力学的对比如\cref{tab:duality} 所示。
...
从"揭示"到"创造"的本体论跃迁如\cref{tab:ontology} 所示。
...
若将 $\theta$ 诠释为宇宙总物质密度参数 $\Omega_m$，理论值与 Planck 2018 观测值的对比如\cref{tab:observation} 所示。
...
考察 $2\cos(2\pi/p)$ 在代数数论中的"身份"，代数复杂度门槛如\cref{tab:algebraic} 所示。
...
3、5、7 在 CSQIT 中的层级角色如\cref{tab:threefiveoseven} 所示。
...
将代数扩张次数 $d = (p-1)/2$ 从 1 延续至无穷，物质密度 $\theta(p) = 1/(2+2\cos(2\pi/p))$ 呈现单调递减序列，渐近于 0.25。代数扩张谱系如\cref{tab:expansion_spectrum} 所示。
...
作用量各项的对应关系（W2/W3 层诠释）如\cref{tab:action} 所示。
...
我们严格区分三个认知层次，W1/W2/W3 三层认知体系如\cref{tab:wlevels} 所示。
...
具体分层示例（W1/W2/W3 在各文件中的体现）如\cref{tab:layering} 所示。
...
\textbf{P0 级（核心）}开放问题如\cref{tab:p0} 所示。
...
\textbf{P1 级（重要）}开放问题如\cref{tab:p1} 所示。
...
\textbf{P2 级（长远）}开放问题如\cref{tab:p2} 所示。
...
初始版本包含大量占位符，涵盖公理体系、有限模型和动力学框架，阶段一 sorry 分布如\cref{tab:sorry1} 所示。

\begin{table*}[!t]
\centering
\scriptsize
\begin{tabular}{p{5.5cm}p{1cm}p{8cm}}
\hline
文件 & sorry 数量 & 内容 \\
\hline
...
消除代数因果序和有限模型的关键证明，阶段二 sorry 消除情况如\cref{tab:sorry2} 所示。
...
攻克最困难的有限模型证明，阶段三 sorry 消除情况如\cref{tab:sorry3} 所示。
...
当前状态（v11.2.0）如\cref{tab:current} 所示。
...
关键定理与代码位置如\cref{tab:theorems} 所示。

\begin{table*}[!t]
\centering
\scriptsize
\begin{tabular}{p{2.2cm}p{5cm}p{6.5cm}p{0.8cm}}
\hline
定理 & 文件 & Lean 名称 & 层次 \\
\hline
...
\textbf{判据}：素数 $p$ 的代数扩张次数 $d=(p-1)/2$ 决定因果格的复杂性层级。素数筛法的扩张谱系如\cref{tab:sieve} 所示。
...
此表以\textbf{最简形式}呈现：每条 CSQIT 公理 $\to$ 对应的物理原理 $\to$ 在标准理论中的角色。作为正文的"物理动机索引"。公理与物理原理的对照如\cref{tab:axiom_physics} 所示。
...
各素数 $p$ 的定性相图如\Cref{tab:phase_diagram} 所示，展示代数复杂性如何决定宇宙学模态、结构形成与观测者涌现。
...
从宇宙学到认知尺度的跨尺度链条如\Cref{tab:cross_scale} 所示，表明 Fin 7 不是"出现在某一层的特殊数字"，而是贯穿所有生长层级的代数不变量。
...
这是跨尺度链条中最令人震惊的环节——\textbf{遗传密码子（64 种）与 Fin 8 的完备闭包（$8^2=64$）}的精确对应如\cref{tab:genetic} 所示。
...
\centering
\scriptsize
\begin{tabular}{p{1.2cm}p{0.8cm}p{4.5cm}p{5cm}p{4cm}}
\hline
...
\boxed{
\begin{gathered}
\text{饱和：} 8 \xrightarrow{8^2} 64 \xrightarrow{\text{紧化}} \infty \\
\text{（平方递推，完全闭包} \to \text{连续极限）}
\end{gathered}
}
\]
}
...
\boxed{
\begin{aligned}
&\text{时间} = \text{从64到}\infty\text{的追踪参数} \\
&\text{方向4} = \text{三维空间的内在四重对称性}
\end{aligned}
}
\]
}
...
感谢 TRAE AI 与 DeepSeek AI 在 Lean 4 形式化证明与文稿撰写方面提供的强力高效协助。本工作为独立研究，无外部资助。作者声明无利益冲突。完整源代码、形式化证明及补充材料可在 \url{https://github.com/New-Beginning-Universe-Research-Group/CSQIT} 公开获取。
