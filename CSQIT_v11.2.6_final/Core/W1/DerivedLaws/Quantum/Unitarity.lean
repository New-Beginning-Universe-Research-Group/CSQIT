...
文件: Core/W1/DerivedLaws/Quantum/Unitarity.lean
版本: v11.6.0
日期: 2026-07-19
...
依赖层级：🔵 W1 严格定理
  前提：AxiomA（编织公理） + AxiomC（振幅公理）
  直接从 AxiomC 的 norm_one 公理导出
...
而"模长平方 = 概率"是额外的物理对应。
-/
def probability_conservation_interpretation : Prop :=
  ∀ (α : C), Complex.normSq (AxiomC.amplitude α) = 1 →
  "|amplitude α|² 可以诠释为过程 α 的概率" := by
  intro α h
  rfl
