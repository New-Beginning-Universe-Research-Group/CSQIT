...
物理意义：
  ...
  位置序低 → 粒子排列无序（气体）
-/
noncomputable def positionalOrder (S : Finset X) : ℝ :=
  let n := S.card
  if n ≤ 1 then 1.0
  else
    let avg_dist := (∑ x ∈ S, ∑ y ∈ S, dist psi_pos x y) / ((n * n : ℕ) : ℝ)
    1 / (avg_dist + 1)
...
物理意义：
  ...
  取向序低 → 自旋取向无序（顺磁体、气体）
-/
noncomputable def orientationalOrder (S : Finset X) : ℝ :=
  let n := S.card
  if n = 0 then 0.0
  else
...
固体具有长程有序性（简化表述）。
-/
theorem solid_long_range_order (S : Finset X) (h_solid : isSolid psi_pos psi_spin S) :
    True := by
  trivial
...
液体具有短程有序性（简化表述）。
-/
theorem liquid_short_range_order (S : Finset X) (h_liquid : isLiquid psi_pos psi_spin S) :
    True := by
  trivial
...
气体具有长程无序性（简化表述）。
-/
theorem gas_no_long_range_order (S : Finset X) (h_gas : isGas psi_pos psi_spin S) :
    True := by
  trivial
...
注：这是一个简化模型，
    真实相变温度还与压强等因素有关。
-/
noncomputable def meltingPoint (S : Finset X) : ℝ :=
...
注：这是一个简化模型。
-/
noncomputable def boilingPoint (S : Finset X) : ℝ :=
