/-
CSQIT v11.5 — 引力常数的群论推导
文件: Core/v115/GravityDerivation.lean
版本: 11.5.0
日期: 2026-07-12
...
/-- 全闭包分母 = lcm(三群阶) / 2 = 420 -/
def totalClosure : ℕ :=
  Nat.lcm (Nat.lcm G1_order G2_order) G3_order / 2
...
theorem three_locks_sum :
    baryonNum + darkMatterNum + darkEnergyNum = totalClosure := by
  rfl
...
theorem v115_eq_v11_weavingStiffness :
  ...
  have h3 : (totalClosure : ℝ) = (V11Ref.lock2_totalClosure : ℝ) := by
  ...
  have h_main : weavingStiffness =
      inverseAlpha * observerBridge * (totalClosure : ℝ) / (darkEnergyNum : ℝ) := by
    rfl
