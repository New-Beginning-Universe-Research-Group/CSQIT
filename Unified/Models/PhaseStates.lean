...
noncomputable def positionalOrder (psi_pos : X → ℝ) (S : Finset X) : ℝ :=
  if S.card ≤ 1 then (1 : ℝ)
  else
    1 / ((∑ x ∈ S, ∑ y ∈ S, dist x y) / ((S.card * S.card : ℕ) : ℝ) + 1)
...
noncomputable def orientationalOrder (psi_spin : X → ℂ) (S : Finset X) : ℝ :=
  if S.card = 0 then (0 : ℝ)
  else
    ((∑ x ∈ S, Complex.re (psi_spin x)) / (S.card : ℝ))^2 +
    ((∑ x ∈ S, Complex.im (psi_spin x)) / (S.card : ℝ))^2
...
theorem positionalOrder_nonneg (psi_pos : X → ℝ) (S : Finset X) :
  ...
  · unfold positionalOrder
    simp only [if_pos h]
    norm_num
  · have h_gt : 1 < S.card := by linarith
    unfold positionalOrder
    simp only [if_neg h]
    have h_dist_nonneg : ∀ x y : X, 0 ≤ dist x y := by
      intro x y; unfold dist; split_ifs <;> norm_num
    have h_sum_nonneg : 0 ≤ ∑ x ∈ S, ∑ y ∈ S, dist x y :=
      Finset.sum_nonneg (fun x _ => Finset.sum_nonneg (fun y _ => h_dist_nonneg x y))
    have h_nn_nonneg : 0 ≤ ((S.card * S.card : ℕ) : ℝ) := by exact_mod_cast Nat.zero_le _
    have h_avg_nonneg : 0 ≤ (∑ x ∈ S, ∑ y ∈ S, dist x y) / ((S.card * S.card : ℕ) : ℝ) :=
      div_nonneg h_sum_nonneg h_nn_nonneg
    have h_denom_pos : (0 : ℝ) < (∑ x ∈ S, ∑ y ∈ S, dist x y) / ((S.card * S.card : ℕ) : ℝ) + 1 := by
      linarith
...
theorem melting_lt_boiling (psi_pos : X → ℝ) (S : Finset X)
    (h_pos : 0 < positionalOrder psi_pos S) :
    meltingPoint psi_pos S < boilingPoint psi_pos S := by
  unfold meltingPoint boilingPoint
  have h1 : 0 < positionalOrder psi_pos S := h_pos
  have h2 : positionalOrder psi_pos S * 100 < positionalOrder psi_pos S * 200 := by
    nlinarith
  exact h2
...
theorem solid_liquid_exclusive (psi_pos : X → ℝ) (psi_spin : X → ℂ) (S : Finset X) :
  ...
  intro h
  have h_solid : isSolid psi_pos psi_spin S := h.1
  have h_liquid : isLiquid psi_pos psi_spin S := h.2
  have h1 : positionalOrder psi_pos S > 0.9 := h_solid.1
  have h2 : positionalOrder psi_pos S ≤ 0.9 := h_liquid.2.1
  linarith

theorem liquid_gas_exclusive (psi_pos : X → ℝ) (psi_spin : X → ℂ) (S : Finset X) :
    ¬ (isLiquid psi_pos psi_spin S ∧ isGas psi_pos psi_spin S) := by
  intro h
  have h_liquid : isLiquid psi_pos psi_spin S := h.1
  have h_gas : isGas psi_pos psi_spin S := h.2
  have h1 : positionalOrder psi_pos S > 0.5 := h_liquid.1
  have h2 : positionalOrder psi_pos S ≤ 0.5 := h_gas.1
  linarith
