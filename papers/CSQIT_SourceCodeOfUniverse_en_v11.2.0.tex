...
\usepackage{pifont}
\usepackage{booktabs}
\usepackage{hyperref}  % 必须在其他包之后加载
\usepackage{cleveref}  % 必须在 hyperref 之后

% 修复 revtex4-2 4.2f 初始化 bug：collaboration@sw 等开关未被预初始化
\makeatletter
\@booleanfalse\collaboration@sw
\makeatother

% revtex4-2 4.2f 缺少 \orcid 命令定义（导言区使用，不产生输出）
\providecommand{\orcid}[1]{\gdef\@orcid{#1}}

\newtheorem{theorem}{Theorem}
\newtheorem{definition}{Definition}

% 标题层级样式（居中，符合APS规范，但放大字号让大段落标题更突出）
\makeatletter
\def\section{%
  \@startsection{section}{1}{\z@}
    {1.0cm \@plus1ex \@minus .2ex}{0.6cm}
    {\normalfont\large\bfseries\centering}%
}
\def\subsection{%
  \@startsection{subsection}{2}{\z@}
    {.9cm \@plus1ex \@minus .2ex}{.5cm}
    {\normalfont\normalsize\bfseries\centering}%
}
\def\subsubsection{%
  \@startsection{subsubsection}{3}{\z@}
    {.8cm \@plus1ex \@minus .2ex}{.5cm}
    {\normalfont\normalsize\itshape\centering}%
}
\makeatother

% 修复 revtex4-2 4.2f 初始化 bug：
% 1. 手动触发 \frontmatter@init 初始化 \@affil@match 等开关
% 2. 立即清空 \frontmatter@init，阻止 \AtBeginDocument 再次执行（否则会重置 \@title 为 "No title"）
\makeatletter
\frontmatter@init
...
\subsection{Approach: Formal Deductive Chain from Axioms to Observation}

The methodology of CSQIT is: starting from information-theoretic axioms, through machine-verifiable formal proofs, derive numerical results that can be compared with observational cosmology. The entire process has zero free parameters. The structure of this deductive chain is as follows:

\begin{enumerate}
    \item \textbf{Source code layer}: 10 axioms (AxiomA--K, with AxiomE removed as it is derivable from AxiomA) define causal relations, rule composition, quantum amplitudes, and dynamical evolution.
    \item \textbf{Compilation layer}: Starting from the axioms, formally prove structural theorems such as the Duality Two-One Theorem and algebraic causal order.
    \item \textbf{Model layer}: Finite models (Fin 5, Fin 7, Fin 8) verify the consistency of the axiom system and generate characteristic values.
    \item \textbf{Output layer}: The characteristic constant $\theta \approx 0.308$ is derived, deviating by approximately 1\% from the $\Omega_m \approx 0.311$ observed by Planck 2018.
\end{enumerate}

\subsection{Methodological Positioning}

Three approaches to constructing physical theories are compared in \cref{tab:methodology}. CSQIT belongs to the third category: its starting point is neither observational data nor geometric intuition, but a minimal set of axioms about ``how information is structured.'' Formalized proofs ensure every step of the logical chain is machine-verifiable, and the agreement between the final numerical output and observation constitutes the empirical anchor.
...
Precise boundary statement: the algebraic derivation from axioms to $\theta$ is a complete W1-level theorem; the correspondence from $\theta$ to $\Omega_m$ involves a W3-level interpretive leap; the convergence from discrete to continuous remains an open problem.

\subsection{Contribution Summary}

This work makes seven key contributions:
...
\begin{enumerate}
    ...
    \item \textbf{Algebraic expansion spectrum}: monotonic decrease of $\theta(p)$ and uniqueness of $p=7$ within the structure formation window
    \item \textbf{Existential inversion (W3)}: structural necessity rather than cosmic choice as the epistemological framing
\end{enumerate}

\subsection{Epistemological Stance (W3)}

The present work adopts a specific epistemological stance: \textbf{observers are internal nodes of the causal lattice, not external spectators}. This has the following consequences:

\begin{enumerate}
    \item What we observe is not ``the universe as it is,'' but rather ``the universe as seen from within a structure of a given algebraic type.''
    \item The characteristic constant $\theta \approx 0.308$ is not a parameter the universe ``chose''; it is a structural invariant of the class of causal lattices capable of supporting internal observers.
    \item The question ``why is the universe the way it is?'' is reframed as ``what algebraic structures can support observers who ask this question?''---and the answer, in the CSQIT framework, is that the algebraic base must be $\mathbb{Z}_7$.
\end{enumerate}
...
The narrative follows a growth path from simple to complex:

\begin{itemize}
    \item \textbf{\S 2 Source Code} (Seed): Axiomatic system---defining the rules of the game
    \item \textbf{\S 3 First Bifurcation}: Duality Two-One Theorem---causality and information cannot both be realized
    \item \textbf{\S 4 Emergence of Causality} (Roots): Algebraic causal order---causality grows out of algebraic structure
    \item \textbf{\S 5 Numerical Anchor} (Trunk): Fin 7 model and the cosmological constant $\theta$---from structure to quantity
    \item \textbf{\S 6 Scale Dynamics} (Branches): Projective compactification and gauge closure---time as a scale parameter
    \item \textbf{\S 7 Arrow of Time} (Branches): Lattice-theoretic derivation of the second law---the necessity of entropy increase
    \item \textbf{\S 8 Finiteness Boundary}: Fundamental limits of finite models---the boundary between the knowable and the unknowable
    \item \textbf{\S 9 Honest Boundaries}: Open problems and incomplete proofs
    \item \textbf{\S 10 Epistemology} (Fruit): The formal status of the internal observer
\end{itemize}
...
\section{Emergence of Causality: Algebraic Causal Order}
...
Consider the algebraic ``identity'' of $2\cos(2\pi/p)$ in algebraic number theory. The algebraic correspondence is summarized in \cref{tab:algebraic}.
...
5 & $(\sqrt{5}-1)/2$ & 2 & $\mathbb{Q}(\sqrt{5})$ & 0.382 & Golden ratio recurrence \\
...
\subsubsection{Expansion Spectrum and Structure Formation Window}

To systematically assess the uniqueness of $p=7$, we define the expansion degree $d(p) = (p-1)/2$ for each prime $p \ge 3$, and compute $\theta(p) = 1/(2+2\cos(2\pi/p))$. The results form a monotonically decreasing sequence, as shown in \cref{tab:expansion_spectrum}:

\begin{table*}[!t]
\centering
\begin{tabular}{ccclc}
\hline
Prime $p$ & $d=(p-1)/2$ & $\theta(p)$ & Structural Character & In (0.28, 0.33)? \\
\hline
3 & 1 & 1.000 & Absolute closure, no evolution & No \\
5 & 2 & 0.382 & Golden ratio recurrence, reversible oscillation & No (above) \\
\textbf{7} & \textbf{3} & \textbf{0.308} & \textbf{Non-linear self-reference, three-body interaction} & \textbf{Yes} \\
11 & 5 & 0.272 & Quintic nonlinearity, structure formation hindered & No (below) \\
13 & 6 & 0.265 & Sextic nonlinearity, dark energy dominated & No (below) \\
17 & 8 & 0.259 & Octic nonlinearity, dilution acceleration & No (below) \\
$\infty$ & $\infty$ & 0.250 & Continuum limit, pure de Sitter & No (below) \\
\hline
\end{tabular}
\caption{Algebraic expansion spectrum: $\theta(p)$ is strictly monotonically decreasing for primes $p \ge 3$. Only $p=7$ falls within the structure formation window $(0.28, 0.33)$.}
\label{tab:expansion_spectrum}
\end{table*}

\textbf{Mathematical property}: $\theta(p)$ is strictly monotonically decreasing for primes $p \ge 3$. As $p \to \infty$, $\theta(p) \to 0.25$.

\textbf{Structure formation window}: Structure formation in the universe requires matter density to lie in a specific range: too low ($\Omega_m < 0.28$) and perturbations cannot collapse into galaxies; too high ($\Omega_m > 0.33$) and the universe recollapses before structure can mature. Only $p=7$ yields $\theta(p) \in (0.28, 0.33)$.

\textbf{Uniqueness statement (W3)}: Within the algebraic expansion spectrum of primes, $p=7$ is the unique prime whose corresponding $\theta(p)$ falls within the structure formation window. This is not a coincidence---it is a constraint from algebraic number theory on what kinds of causal lattices can support complex structure.
...
\subsection{Existential Inversion: From ``Choice'' to ``Existence Condition''}
...
\textbf{Traditional framing}: ``The universe chose 7. Why?'' \\
\textbf{CSQIT framing (W3)}: ``Structure$=7$ is the necessary condition for observers who can ask `why 7' to exist.''

We call this reframing \textbf{existential inversion}.

\subsubsection{Formal Statement (W3)}
...
\begin{itemize}
    \item For $p=3$: The lattice is absolutely closed ($\theta=1$). No evolution, no time arrow, no observers.
    \item For $p=5$: The lattice is in golden ratio recurrence ($\theta \approx 0.382$). Reversible oscillation, matter density too high for stable structure formation, no irreversible records, no observers who can ``remember'' why.
    \item For $p=7$: The lattice has cubic nonlinearity and three-way weaving flow. Past-present-future form a closed loop, enabling irreversible records and self-referential cognition.
    \item For $p \ge 11$: $\theta(p) < 0.272$, too low for stable structure formation. Density perturbations grow too slowly, galaxies cannot form within the cosmic age, no bound structures, no observers.
\end{itemize}

\begin{quote}
\textbf{Syntactic compression}: It is not that the universe chose 7; rather, it is because structure$=7$ that we became ``we.''
\end{quote}
...
\subsection*{Time as Scale, Not Dimension (W3)}

Time is not a fundamental dimension---it is the scalar parameter of the causal lattice's refinement trajectory from finite to infinite. Space has 3 dimensions (rooted in the three real roots of the cubic extension), but time is not a ``fourth dimension''---it is the \textbf{tracking parameter of projective compactification}.

\textbf{Structural meaning of 3+1}:
\[
\begin{aligned}
&\text{Spacetime} = 3 + 1 \\
&\iff \text{stable closure (3)} + \text{projective tracking (1)}
\end{aligned}
\]
...
\section{Arrow of Time: Lattice-Theoretic Derivation of the Second Law}
...
\noindent\textbf{Precise statement of the information causality bound}: The causal monotonicity of entropy (AxiomI) implies a \textbf{cardinality bound on the entropy of finite causal sets} ($|S| \le |M|$), which is a \textit{combinatorial} counterpart of the Bekenstein bound~\cite{Bekenstein1973}. We do \textbf{not} claim to have derived the area law of black hole entropy; the relation between these two structures---if any---is an open problem at the W2/W3 level.
...
The three levels of certainty (W1/W2/W3) are summarized in \cref{tab:levels}.
...
The formalization of CSQIT is an iterative engineering effort spanning many versions. From the very first version, the project underwent countless rewrites---from early AI-assisted manual operations to later systematic AI workflows with parallel sub-agents---the total number of \texttt{sorry} and \texttt{admit} statements eliminated over the course of the project is beyond precise accounting. This section documents only the \textbf{traceable sorry elimination history since v11.0} as a methodological snapshot of the formalization engineering process.

Since v11.0, 13 \texttt{sorry} statements were eliminated through formalized proofs. 4 \texttt{sorry} statements remain intentionally in \texttt{cyclic\_stable\_substructure} as counterexamples marking mathematically impossible assertions.

\subsection{Build Statistics}

The full formalization is implemented in Lean 4 version v4.29.0-rc6, comprising 50 Lean files with approximately 24,300 lines of formal proof. The entire codebase is machine-verified via \texttt{lake build} with 2196 compilation tasks and 0 errors.
...
\section{Four Closure Loops and the Extended Unified Identity Equation}
...
\begin{itemize}
    ...
    \item \textbf{Direction 4 tetrahedral symmetry (W3)}: Beyond the 8 generators of SU(3), there is a deeper symmetry correspondence---\textbf{Direction 4}. The root of spatial direction in 3D space is not 3 orthogonal axes but the \textbf{4 vertex directions of a regular tetrahedron}. This fourfold symmetry has consistent correspondences across scales:
    \begin{itemize}
        \item \textbf{Geometric layer}: 4 vertices of a regular tetrahedron = the most symmetric finite point set in 3D space
        \item \textbf{Gauge layer}: 4 generators of SU(2)$\times$U(1) = electroweak unification degrees of freedom
        \item \textbf{Chemical layer}: Carbon's sp$^3$ hybridization = 4 covalent bond directions
        \item \textbf{Life layer}: 4 DNA bases (A, T, C, G) = basic letters of genetic information
    \end{itemize}
...
\subsection{Closure Loop 4: Life-Cognition Closure Loop (Micro $\to$ Macro, W3)}

Extending the algebraic structure to atomic, molecular, and life scales, we observe consistent cross-scale correspondences:

\begin{itemize}
    \item \textbf{Atomic scale}: The cyclic generator of $\mathbb{Z}_7$ corresponds to the periodic structure of electron shell filling (period lengths 2, 8, 8, 18, 18, 32). The chemical properties of elements are determined by the ``weaving order'' of their outermost electrons.
    \item \textbf{Molecular scale}: Types of molecular bonds correspond to basic CSQIT operations---covalent bonds correspond to \texttt{compose} rule composition, ionic bonds correspond to \texttt{combine} information fusion, and hydrogen bonds correspond to weak associations of partial weaving.
    \item \textbf{Life scale}: The 64 genetic codons precisely correspond to the complete closure of $\text{Fin}\,8$ ($8^2 = 64$). Among them, 61 encode amino acids (non-zero amplitude, corresponding to visible matter) and 3 are stop codons (zero amplitude, corresponding to dark matter).
    \item \textbf{Cognitive scale}: Observers, as self-referential nodes capable of recording irreversible histories and asking structural questions, have their existence as a sufficient condition for algebraic structure being 7---this is existential inversion.
\end{itemize}

\textbf{Precise statement}: From $\Omega_m$ to DNA, structure$=7$ is the algebraic invariant across all scales. The combinatorial structure of the causal lattice not only determines the universe's macroscopic parameters but also encodes all possible weaving patterns from atoms and molecules to life and cognition.

\subsection{The Extended Unified Identity Equation}

Substituting the four closure loops into the CSQIT axiomatic system, we obtain the following \textbf{extended unified identity equation}:

\[
\boxed{
\begin{aligned}
&\Omega_m \equiv \theta(7) \approx 0.308 \;\; \text{(cosmological)} \\
&\text{Periodic table} \iff \text{Fin 7 shell filling} \;\; \text{(atomic)} \\
&\text{Molecular bonds} \iff \texttt{compose}+\texttt{combine} \;\; \text{(molecular)} \\
&64\text{ codons} \iff 8^2\text{ weaving pairs} \;\; \text{(life)} \\
&\text{Observer existence} \iff \text{structure}=7 \;\; \text{(cognitive)}
\end{aligned}
}
\]
...
\begin{enumerate}
    \item \textbf{Duality}: Causality and quantum information are two projections of the same reality---cannot both be nontrivial
    \item \textbf{Algebraic causality}: Causal relations emerge from algebraic structure---causal closure = algebraic closure
    \item \textbf{Projective compactification}: Infinity is a cyclic compactification, not a boundary---the universality of $\pi$ has topological roots
\end{enumerate}

These insights have mathematical support (W1 layer theorems), but their physical interpretation belongs to W2/W3 layers.

The proximity of $\theta \approx 0.308$ to $\Omega_m = 0.311$ is the first observational anchor of the structural deduction. This is not a prediction (because $\theta = \Omega_m$ is an interpretive leap), not a coincidence (because it is the unique value derived from axioms with zero free parameters), but an anchor indicating that this deductive chain may be related to real physics.

\textbf{Epistemic implication of existential inversion (W3)}:

CSQIT is not a ``model of the universe.'' It is a \textbf{deductive framework for ``how observers are possible''}. The ultimate question it answers is not ``What is the universe like?'' but rather the more fundamental: ``What kind of universe can produce observers who ask this question?''

The answer to this question, formally expressed as the unique nontrivial root of the cubic equation $\theta^3 - 6\theta^2 + 5\theta - 1 = 0$ at $\theta \approx 0.308$, observationally as Planck data $\Omega_m \approx 0.311$, and epistemologically as: the reason we see 0.308 is because structure$=7$ made us ``us.''

A golden ratio ($\mathbb{Z}_5$) universe has no irreversible records and thus cannot produce observers. A cubic nonlinear ($\mathbb{Z}_7$) universe possesses the nonlinear closed loop of ``past-present-future,'' making ``questioning'' and ``recording'' possible. The reason we can ask ``why 7'' is precisely because we are ourselves products of 7.

To our knowledge, this is the first implementation in a discrete causal framework of: complete formal deduction from axiomatic system to characteristic constant; numerical derivation with zero free parameters; comparability with observational cosmology; cross-scale structural correspondence from atoms to life; and internal self-referential constraint on observer existence conditions.

Whether this path leads to ultimate truth remains unknown. But it at least demonstrates that physics need not presuppose an external observer but can instead start from the condition that observers are internal nodes of the structure and derive values consistent with observation.
...
\section{Six-Layer Uniqueness Lock-in Framework}

\subsection{Positioning}

This appendix presents the \textbf{six-layer uniqueness lock-in framework} of CSQIT, integrating mathematical rigor, physical correspondence, and epistemological framing. It is not ``one possible model among many,'' but rather \textbf{the algebraic form that any causal-information closed system capable of producing irreversible observers must necessarily take}, within the axiomatic constraints of the theory.

Each layer is explicitly labeled with W1/W2/W3 certainty levels; all cross-layer assertions are declared.

\subsection{The Logical Chain of Six Lock-ins}

\subsubsection*{First Lock-stop: Axiomatic Closure (W1)}

\textbf{Theorem} (\texttt{input\_must\_be\_empty}, \texttt{Core/CausalWeaving.lean}): In every model satisfying AxiomA, the input list of every rule is empty.
...
\subsubsection*{Second Lock-stop: Two-Aspect Conflict (W1)}
...
\textbf{Code support}: The comparison between \texttt{fin7Model} and \texttt{fin8Model} verifies the necessity of prime order.

\subsubsection*{Third Lock-stop: Prime Sieve --- The Unique Gate of the Extension Spectrum (W3)}
...
2 & 5 & $C_2$ & 0.382 & Eternal recurrence & \ding{55} No irreversible record \\
\textbf{3} & \textbf{7} & \textbf{$C_3$} & \textbf{0.308} & \textbf{Historical evolution} & \textbf{\ding{51} Uniquely viable} \\
5 & 11 & $C_5$ & 0.272 & Accelerating void & \ding{55} No structure \\
...
\subsubsection*{Fourth Lock-stop: Gauge Projection --- Fin 8 Forces SU(3) (W3)}
...
\textbf{Mod-8 congruence lock-stop}: Global conservation 15 mod 8 yields 7. Hence the linear conservation layer (Fin 8) of gauge symmetry necessarily projects to the matter-density layer (Fin 7) under modular arithmetic.
...
\subsubsection*{Fifth Lock-stop: Triple Anchoring --- The Unique Value of $\theta$ (W2/W3)}
...
\textbf{Structure-formation anchoring} (W2): If $\Omega_m > 0.33$ (e.g., $p=5$ yielding 0.382), the universe closes before radiation-matter equality, preventing large-scale structure; if $\Omega_m < 0.28$ (e.g., $p\geq 11$), density perturbations grow insufficiently for galaxies to form within the age of the universe.
...
\subsubsection*{Sixth Lock-stop: Self-Referential Cognition --- The Unique Proof of Observer Existence (W3)}

\textbf{Epistemological inversion}: Synthesizing the first five lock-ins yields a closed-loop inference:

\begin{enumerate}
    \item \textbf{The first five lock-ins prove}: any causal lattice capable of producing irreversible records must be Fin 7.
...
\subsection{Unified Identity Equation of the Six Lock-ins}
...
&\text{Two-aspect conflict} \implies \text{prime-order cyclic} \\
&\text{Extension spectrum} \implies p=7 \;(d=3) \\
&\text{Fin 8 closure} \implies \text{SU(3) projection} \\
&\text{Triple anchoring} \implies \theta = 0.308 \equiv \Omega_m \\
&\text{Self-ref. closure} \implies \text{Observer} \iff \text{Str.}=7
\end{aligned}
}
$$
...
Of the six lock-ins in this appendix, \textbf{the first and second layers are W1 proven theorems}; \textbf{the third through sixth layers are W3 interpretations}, each supported by W1 theorems (\texttt{order\_jump\_example}, \texttt{BV\_ratio\_from\_EffectiveFin7}, \texttt{input\_must\_be\_empty}). This is an \textbf{epistemological closure}, not a mathematical theorem --- its strength depends on the reasonableness of W3 interpretations, but its internal logical chain is closed.
...
The qualitative phase diagram across primes $p$ is summarized in \cref{tab:phase_diagram}, showing how algebraic complexity determines cosmological modality, structure formation, and observer emergence.
...
5 & 0.382 & 2 & Eternal recurrence: reversible oscillation & \ding{55} & \ding{55} \\
\textbf{7} & \textbf{0.308} & \textbf{3} & \textbf{Historical evolution: irreversible info} & \textbf{\ding{51}} & \textbf{\ding{51}} \\
11 & 0.272 & 5 & Accelerating void: structure dilution & \ding{55} & \ding{55} \\
...
This appendix presents the \textbf{cross-scale structural extension} of the CSQIT framework: it argues that the same Fin 7 / Fin 8 algebraic structure that corresponds to the cosmological matter density $\Omega_m \approx 0.308$ also encodes the \textbf{periodic table shell-filling, molecular-bond weaving types, genetic-code closure, and the cognitive self-reference condition}. All claims in this appendix are \textbf{W3-level physical interpretations} explicitly labeled as such; they do not add new mathematical assertions to the W1 layer, but re-interpret already-formalized structures (Fin 8 closure, Fin 7 characteristic constant, mod-8 congruence) at successive physical scales.
...
The cross-scale chain from cosmological to cognitive scales is summarized in \cref{tab:cross_scale}, demonstrating that Fin 7 is not a ``special number appearing at a particular layer'' but an algebraic invariant running through all growth layers.
...
\[
\boxed{\begin{aligned}
&\text{Genetic code} = \text{Fin 8 complete closure} \\
&\quad\text{projected onto Fin 7 matter classification}
\end{aligned}}
\]
...
&\text{Molecular bonds} \iff \texttt{compose}+\texttt{combine} \;\; \text{(molecular)} \\
&64\text{ codons} \iff 8^2\text{ weaving pairs} \;\; \text{(life)} \\
&\text{Observer exists} \iff \text{structure}=7 \;\; \text{(cognitive)}
\end{aligned}
}
\]
...
\hline
0 & 0 & \texttt{input\_must\_be\_empty} & Core/CausalWeaving.lean & Empty weaving \\
...
\boxed{
\begin{aligned}
&\text{Generation: } 0 \xrightarrow{1} 2 \xrightarrow{2} 4 \xrightarrow{2} 8 \\
&\quad\text{(powers of 2, stable closure)}
\end{aligned}
}
\]
...
\boxed{
\begin{aligned}
&\text{Saturation: } 8 \xrightarrow{8^2} 64 \xrightarrow{64^2} \infty \\
&\quad\text{(square recursion, full closure } \to \text{ continuum limit)}
\end{aligned}
}
\]
...
\boxed{
\begin{aligned}
&\text{Why no 16 and 32? Because they are} \\
&\text{algebraically forced out --- no composite-order} \\
&\text{lattice can carry a unitary and injective} \\
&\text{causal-information structure.}
\end{aligned}
}
\]
...
\boxed{
\begin{aligned}
&\text{Time} = \text{tracking parameter from 64 to } \infty \\
&\text{Direction 4} = \text{intrinsic four-fold symmetry of 3D space}
\end{aligned}
}
\]
